...
let dashboard_methods = DashboardViewVue.methods
...
export default defineComponent({
    ...
    methods : {
        async createWorksession(){
            let session = document.querySelector('#session_name') as HTMLInputElement
            let session_name = session.value
            ...
            if (local_user != null) {
                ...
                let result = await axios.post('http://localhost:5000/worksessions', new_workout, request_config)                
        
                if (result.data){
                    console.log(result.data)
                    this.new_session_id = result.data.id
                    this.new_session_name = result.data.name
                    let content_ctnr = document.querySelector('.content_ctnr') as HTMLElement
                    content_ctnr.style.flexFlow = 'column-reverse'
                }
            }
        },
        async createExercise(event: Event){
            event.preventDefault()
            let name = document.querySelector('#exercise_name') as HTMLInputElement 
            let rep_count = document.querySelector('#exercise_reps') as HTMLInputElement
            let weight = document.querySelector('#exercise_weight') as HTMLInputElement
            let exercise_unit = document.querySelector('#exercise_unit') as HTMLInputElement
            ...
            if (local_user != null) {
                ...
                let result = await axios.post('http://localhost:5000/exercises', new_exercise, request_config)
                if (result.data){
