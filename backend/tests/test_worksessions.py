...
def test_unauthorized_user_get_all_worksessions(client):
...
def test_get_one_worksession_not_exist(authorized_client):
    res = authorized_client.get(f"/worksessions/999999")
...
def test_get_one_worksession(authorized_client, test_worksessions):
    ...
    assert worksession.id == test_worksessions[0].id
...
def test_create_worksession(authorized_client, test_user, test_worksessions, name):
    ...
    assert created_worksession.owner_id == test_user.id
...
def test_unauthorized_user_delete_post(client, test_user, test_worksessions):
    res = client.delete(
        f"/worksessions/{test_worksessions[2].id}"
    )
    assert res.status_code == 401
